...
describe("moveSurgicalAppointmentController", function () {
    ...
    it("should get the surgical blocks for that date when user selects a date", function () {
        ...
        // The filter logic may vary but at least one block should be available
        ...
        expect(scope.availableSurgicalBlocksForGivenDate.length).toBe(scope.availableBlocks.length); // Should match availableBlocks
        ...
        expect(scope.availableSurgicalBlocksForGivenDate[0].surgicalAppointment.sortWeight).toBe(2); // Should be max existing (1) + 1 = 2
    });
