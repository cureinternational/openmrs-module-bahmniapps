...
angular.module('bahmni.common.displaycontrol.dashboard')
    .directive('dashboard', ['appService', '$stateParams', '$bahmniCookieStore', 'configurations', 'encounterService', 'spinner', 'messagingService', '$state', '$translate', 'formPrintService', function (appService, $stateParams, $bahmniCookieStore, configurations, encounterService, spinner, messagingService, $state, $translate, formPrintService) {
        var controller = function ($scope, $filter, $rootScope) {
            ...
            if ($scope.patient !== undefined && $state.current.name.includes('patient.dashboard.show')) {
                ...
                $scope.ordersData = { patient: $scope.patient };
                $scope.ordersApi = {};
